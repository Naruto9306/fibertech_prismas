...
//       if (routeData) {
//         // En lugar de abrir un modal, navegamos al mapa con la ruta seleccionada
//         router.push({
//           pathname: '../map',
//           params: { selectedRouteId: routeId.toString() }
//         });
//       } else {
//         Alert.alert('Error', 'No se encontró información de la ruta');
//       }
...
export default function ProjectsScreen() {
  ...
  const loadProjects = async () => {
    try {
      ...
      // Verificar si la tabla projects existe
      const tableExists = await (db as any).getFirstAsync(
        "SELECT name FROM sqlite_master WHERE type='table' AND name='projects'"
      );
      
      if (tableExists) {
        // Obtener proyectos con información de la ruta
        const projectsData = await (db as any).getAllAsync(`
          SELECT p.*, r.name as route_name 
          FROM projects p 
          LEFT JOIN routes r ON p.route_id = r.id 
          ORDER BY p.created_at DESC
  ...
  const openRouteDetails = async (routeId: number) => {
    try {
      ...
      if (Platform.OS === 'web') {
        Alert.alert('Info', 'Funcionalidad no disponible en web');
        return;
      }
      
      const routeData = await (db as any).getFirstAsync(
        'SELECT * FROM routes WHERE id = ?',
        [routeId]
      );
        ...
        },
        {
          ...
          onPress: async () => {
            try {
              ...
              await (db as any).runAsync('DELETE FROM projects WHERE id = ?', [projectId]);
              Alert.alert('Éxito', 'Proyecto eliminado correctamente');
              loadProjects(); // Recargar la lista
            } catch (error) {
              console.error('Error deleting project:', error);

...
// // type Node = NDFNode | PedestalNode | IDFNode | UnitNode;
...
// interface PedestalNode extends BaseNode {
//   type: 'pedestal';
//   // Solo es un punto de conexión, sin detalles adicionales
// }
...
//   // Crear base de datos mock para desarrollo
//   const createMockDatabase = () => {
//     return {
//       withTransactionSync: (callback: () => void) => {
//         console.warn('Mock withTransactionSync');
...
//           ...baseNode,
//           type: 'NDF',
//           fiberCount: parseInt(nodeDetails.fiberCount) || 0,
//           fiberColor: nodeDetails.fiberColor,
//           equipment: nodeDetails.equipment
//         };
//         break;
//       case 'pedestal':
...
//     // Si ya hay nodos, crear una conexión automática con el último nodo
//     if (currentNodes.length > 0) {
//       const lastNodeIndex = currentNodes.length - 1;
//       const newConnection: Connection = {
//         routeId: 0,
//         fromNodeId: lastNodeIndex,
//         toNodeId: currentNodes.length, // El nuevo nodo será el siguiente
//         cableType: 'fibra',
//         length: 0, // Se calcularía la distancia real entre puntos
//         pathType: 'subterranea',
//         status: 'activo'
//       };
//       setCurrentConnections(prev => [...prev, newConnection]);
...
//             {/* Conexiones de la ruta actual */}
//             {currentConnections.map((connection, index) => {
//               const fromNode = currentNodes[connection.fromNodeId];
//               const toNode = currentNodes[connection.toNodeId];
...
//               <Text style={styles.selectorTitle}>Tipo de Nodo:</Text>
//               <View style={styles.selectorButtons}>
//                 {(['NDF', 'pedestal', 'IDF', 'Unit'] as Node['type'][]).map(type => (
//                   <TouchableOpacity
//                     key={type}
//                     style={[
//                       styles.nodeTypeButton,
...
//               {(selectedNodeType === 'NDF' || selectedNodeType === 'Unit') && (
//                 <>
//                   <TextInput
//                     style={styles.input}
//                     placeholder="Cantidad de fibras"
//                     value={nodeDetails.fiberCount}
//                     onChangeText={text => setNodeDetails({...nodeDetails, fiberCount: text})}
//                     keyboardType="numeric"
//                   />
//                   <TextInput
//                     style={styles.input}
//                     placeholder="Color de fibra"
//                     value={nodeDetails.fiberColor}
//                     onChangeText={text => setNodeDetails({...nodeDetails, fiberColor: text})}
//                   />
//                   <TextInput
//                     style={styles.input}
//                     placeholder="Equipos de conectividad"
//                     value={nodeDetails.equipment}
//                     onChangeText={text => setNodeDetails({...nodeDetails, equipment: text})}
//                   />
//                 </>
...
import { 
  ...
  getNodesByRoute,
  getConnectionsByRoute,
  type Route as DatabaseRoute,
  type Node as DatabaseNode,
  type Connection as DatabaseConnection
} from '../../services/database';
import * as Permissions from 'expo-permissions';

// Renombrar las interfaces locales para evitar conflictos
interface BaseNode {
  id?: number;
  routeId: number;
  type: 'NDF' | 'pedestal' | 'IDF' | 'Unit';
...
type LocalNode = NDFNode | PedestalNode | IDFNode | UnitNode;

// Interface para conexiones locales
...
export default function MapScreen() {
  ...
  const [mapType, setMapType] = useState<'standard' | 'satellite' | 'hybrid'>('standard');
  const [isCreatingRoute, setIsCreatingRoute] = useState(false);
  const [currentNodes, setCurrentNodes] = useState<LocalNode[]>([]);
  const [currentConnections, setCurrentConnections] = useState<LocalConnection[]>([]);
  const [routeName, setRouteName] = useState('');
  const [showSaveModal, setShowSaveModal] = useState(false);
  const [showRoutesModal, setShowRoutesModal] = useState(false);
  const [savedRoutes, setSavedRoutes] = useState<DatabaseRoute[]>([]);
  const [selectedRoute, setSelectedRoute] = useState<DatabaseRoute | null>(null);
  const [selectedNodes, setSelectedNodes] = useState<LocalNode[]>([]);
  const [selectedConnections, setSelectedConnections] = useState<LocalConnection[]>([]);
  const [db, setDb] = useState<any>(null);
  const [selectedNodeType, setSelectedNodeType] = useState<LocalNode['type']>('NDF');
  const [showNodeModal, setShowNodeModal] = useState(false);
  const [tempNode, setTempNode] = useState<{latitude: number; longitude: number} | null>(null);
  const [nodeDetails, setNodeDetails] = useState({
    description: '',
...
const requestAndroidPermissions = async () => {
  try {
    ...
    // También solicitar permisos de almacenamiento si necesitas acceder a la base de datos
...
//     const tableCheck = await database.getFirstSync(
...
//     const tableInfo = await database.getAllSync("PRAGMA table_info(routes)");
//     const hasUpdatedAt = tableInfo.some((column: any) => column.name === 'updated_at');
    
//     let query = 'SELECT id, name, created_at as createdAt';
//     if (hasUpdatedAt) {
//       query += ', updated_at as updatedAt';
//     }
//     query += ' FROM routes ORDER BY name';
    
//     const routes = await database.getAllSync(query);
//     setSavedRoutes(routes);
//   } catch (error) {
//     // console.error('Error cargando rutas:', error);
//     console.error('Error cargando rutas:', error instanceof Error ? error.message : String(error));
    
//     // Mostrar alerta solo si es un error crítico
//     // if (error.message.includes('permission') || error.message.includes('access')) {
//     //   Alert.alert(
//     //     'Error de permisos',
//     //     'La aplicación necesita permisos de almacenamiento para acceder a las rutas guardadas',
//     //     [{ text: 'OK' }]
//     //   );
//     // }
...
const loadSavedRoutes = async (database: any) => {
  ...
  try {
    ...
    // Intentar con la nueva estructura primero (created_at/updated_at)
    try {
      const routes = await database.getAllSync(
        "SELECT id, name, created_at as createdAt, updated_at as updatedAt FROM routes ORDER BY name"
      );
      setSavedRoutes(routes);
      return;
    } catch (newStructureError) {
      // Si falla, intentar con la estructura antigua (createdAt/updatedAt)
      console.log('Intentando con estructura antigua de base de datos...');
      
      const routes = await database.getAllSync(
        "SELECT id, name, createdAt, updatedAt FROM routes ORDER BY name"
      );
      setSavedRoutes(routes);
    }
    ...
    // Si hay error de columna faltante, mostrar mensaje específico
    if (error instanceof Error && error.message.includes('no such column')) {
      console.log('Estructura de base de datos incompatible');
      setSavedRoutes([]);
    }
  }
};
  ...
  const saveRoute = async () => {
    ...
    try {
      ...
      // Convertir nodos locales al formato de base de datos
      const databaseNodes: DatabaseNode[] = currentNodes.map(node => ({
        routeId,
        type: node.type,
        latitude: node.latitude,
        longitude: node.longitude,
        description: node.description,
        fiberCount: node.type === 'NDF' || node.type === 'Unit' ? node.fiberCount : 0,
        fiberColor: node.type === 'NDF' || node.type === 'Unit' ? node.fiberColor : '',
        equipment: node.type === 'NDF' || node.type === 'Unit' ? node.equipment : '',
        address: node.type === 'Unit' ? node.address : '',
        client: node.type === 'Unit' ? node.client : '',
        status: node.type === 'Unit' ? node.status : 'activo'
      }));

      // Guardar nodos
      for (const node of databaseNodes) {
        await addNode(db, node);
      }

      // Convertir conexiones locales al formato de base de datos
      const databaseConnections: DatabaseConnection[] = currentConnections.map(conn => ({
        routeId,
      ...
      for (const connection of databaseConnections) {
        await addConnection(db, connection);
      }
  ...
  const loadRoute = async (route: DatabaseRoute) => {
    if (!db || typeof db.getAllSync !== 'function') return;
    ...
    try {
      ...
      // Convertir nodos de la base de datos al formato local
        ...
        switch (node.type) {
          case 'NDF':
            return {
              ...baseNode,
              type: 'NDF',
              fiberCount: node.fiberCount || 0,
              fiberColor: node.fiberColor || '',
          ...
          case 'Unit':
            return {
              ...
              fiberColor: node.fiberColor || '',
              equipment: node.equipment || '',
              status: node.status as 'activo' | 'inactivo' | 'mantenimiento'
            };
          default:
            return baseNode as LocalNode;
        }
      });

      // Convertir conexiones de la base de datos al formato local
      const localConnections: LocalConnection[] = connections.map(conn => ({
        id: conn.id,
  ...
  const getNodeColor = (type: LocalNode['type']) => {
    switch (type) {
      case 'NDF': return '#FF0000'; // Rojo
      case 'pedestal': return '#00FF00'; // Verde
      case 'IDF': return '#0000FF'; // Azul
  ...
  const getConnectionColor = (status: LocalConnection['status']) => {
    return status === 'activo' ? '#00FF00' : '#FF0000';
  };
        ...
        <View style={styles.controlsContainer}>
          ...
          {isCreatingRoute && (
            <View style={styles.nodeTypeSelector}>
              <Text style={styles.selectorTitle}>Tipo de Nodo:</Text>
              <View style={styles.selectorButtons}>
                {(['NDF', 'pedestal', 'IDF', 'Unit'] as LocalNode['type'][]).map(type => (
                  <TouchableOpacity
                    key={type}
                    style={[
                      styles.nodeTypeButton,
                  ...
                  renderItem={({ item }) => (
                    <View style={styles.routeItem}>
                      ...
                      >
                        <Text style={styles.routeText}>{item.name}</Text>
                        <Text style={styles.routeDate}>
                          {item.updatedAt ? new Date(item.updatedAt).toLocaleDateString() : 'Fecha no disponible'}
                        </Text>
                      </TouchableOpacity>
                      <TouchableOpacity 
                        onPress={() => deleteSavedRoute(item.id!)}
